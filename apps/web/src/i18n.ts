...
const DYNAMIC_RULES: ReadonlyArray<readonly [RegExp, string | ((...matches: string[]) => string)]> = [
  ...
  [/^文本 (通过|失败) · 流式 (通过|失败) · 工具 (通过|失败) · 图片 (通过|失败)$/u, (_all, text, streaming, tools, images) => `Text ${text === "通过" ? "passed" : "failed"} · Streaming ${streaming === "通过" ? "passed" : "failed"} · Tools ${tools === "通过" ? "passed" : "failed"} · Images ${images === "通过" ? "passed" : "failed"}`],
