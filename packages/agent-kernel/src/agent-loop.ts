...
export function createCoreToolRegistry(options: CoreToolOptions): AgentToolRegistry {
  ...
  register({ id: "skill.inspect", description: "按需读取一个 Skill 的完整 SKILL.md。", effect: "context", scope: "skill", inputSchema: schema({ id: stringProperty("Skill id。") }, ["id"]), execute: async (args) => options.skills.inspect(requiredString(args.id, "id")) });
